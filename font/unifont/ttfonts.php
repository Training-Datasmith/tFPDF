...
class TTFontFile
{
    ...
    public function seek_table($tag, $offset_in_table = 0): float|int|array
    {
    ...
    public function read_tag(): string|false
    {
        $this->_pos += 4;
    ...
    public function unpack_short($s): int
    {
        $a = (ord($s[0]) << 8) + ord($s[1]);
        if ($a & (1 << 15)) {
            return $a - (1 << 16);
    ...
    public function get_ushort($pos): int
    {
        fseek($this->fh, $pos);
        $s = fread($this->fh, 2);
        return (ord($s[0]) << 8) + ord($s[1]);
    ...
    public function get_ulong($pos): int
    {
        fseek($this->fh, $pos);
        $s = fread($this->fh, 4);
        // iF large uInt32 as an integer, PHP converts it to -ve
    ...
    public function splice($stream, $offset, string $value): string
    {
    ...
    public function get_chunk($pos, $length): string|false
    {
        fseek($this->fh, $pos);
        if ($length < 1) {
            return '';
    ...
    public function get_table(string $tag): string|false
    {
    ...
    public function makeSubset(string $file, &$subset)
    {
        $this->filename = $file;
        $this->fh = fopen($file, 'rb') or die('Can\'t open file ' . $file);
        $this->_pos = 0;
    ...
    public function getGlyphs($originalGlyphIdx, &$start, array &$glyphSet, array &$subsetglyphs): void
    {
        $glyphPos = $this->glyphPos[$originalGlyphIdx];
        $glyphLen = $this->glyphPos[$originalGlyphIdx + 1] - $glyphPos;
        if (!$glyphLen) {
    ...
    public function getHMTX($numberOfHMetrics, $numGlyphs, array &$glyphToChar, $scale): void
    {
    ...
    public function getHMetric($numberOfHMetrics, $gid): string|false
    {
        $start = $this->seek_table('hmtx');
    ...
    public function getLOCA(?string $indexToLocFormat, $numGlyphs): void
    {
    ...
    // CMAP Format 4
    public function getCMAP4($unicode_cmap_offset, array &$glyphToChar, array &$charToGlyph): void
    {
        $this->maxUniChar = 0;
