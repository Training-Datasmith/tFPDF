...
public $maxStrLenRead;
	...
	function getMetrics(string $file): void {
		$this->filename = $file;
	...
	function readTableDirectory(): void {
	...
	function calcChecksum(string $data): array  {
	...
	function get_table_pos($tag): array {
		$offset = $this->tables[$tag]['offset'];
		$length = $this->tables[$tag]['length'];
		return [$offset, $length];
	}

	function seek($pos): void {
		$this->_pos = $pos;
		fseek($this->fh,$this->_pos);
	}

	function skip($delta): void {
		$this->_pos = $this->_pos + $delta;
		fseek($this->fh,$this->_pos);
	}

	function seek_table($tag, $offset_in_table = 0): float|int|array {
	...
	function read_tag(): string|false {
		$this->_pos += 4;
		return fread($this->fh,4);
	}

	function read_short(): int {
		$this->_pos += 2;
		$s = fread($this->fh,2);
		$a = (ord($s[0])<<8) + ord($s[1]);
		if ($a & (1 << 15) ) { return $a - (1 << 16); }
		return $a;
	}

	function unpack_short($s): int {
		$a = (ord($s[0])<<8) + ord($s[1]);
		if ($a & (1 << 15) ) { 
			return $a - (1 << 16); 
		}
	...
	function read_ushort(): int {
		$this->_pos += 2;
		$s = fread($this->fh,2);
		return (ord($s[0])<<8) + ord($s[1]);
	}

	function read_ulong(): int {
		$this->_pos += 4;
		$s = fread($this->fh,4);
	...
	function get_ushort($pos): int {
		fseek($this->fh,$pos);
		$s = fread($this->fh,2);
		return (ord($s[0])<<8) + ord($s[1]);
	}

	function get_ulong($pos): int {
		fseek($this->fh,$pos);
		$s = fread($this->fh,4);
		// iF large uInt32 as an integer, PHP converts it to -ve
		return (ord($s[0])*16777216) + (ord($s[1])<<16) + (ord($s[2])<<8) + ord($s[3]); // 	16777216  = 1<<24
	...
	function pack_short($val): string {
		if ($val<0) { 
			$val = abs($val);
			$val = ~$val;
			$val += 1;
	...
	function splice($stream, $offset, string $value): string {
		return substr((string) $stream,0,$offset) . $value . substr((string) $stream,$offset+strlen($value));
	}
	...
	function get_chunk($pos, $length): string|false {
		fseek($this->fh,$pos);
		if ($length <1) { return ''; }
		return (fread($this->fh,$length));
	}

	function get_table(string $tag): string|false {
	...
	function extractInfo(): void {
		...
		///////////////////////////////////
			...
			if ($names[6])
				...
				$psName = preg_replace('/ /','-',(string) $names[4]);
			else if ($names[1])
				$psName = preg_replace('/ /','-',(string) $names[1]);
			else
				$psName = '';
			if (!$psName)
		...
		$this->stemV = 50 + intval(($usWeightClass / 65.0) ** 2);
...
/////////////////////////////////////////////////////////////////////////////////////////
	...
	function makeSubset(string $file, &$subset) {
		$this->filename = $file;
		$this->fh = fopen($file ,'rb') or die('Can\'t open file ' . $file);
		$this->_pos = 0;
		$this->charWidths = '';
		...
		$post = "\x00\x03\x00\x00" . substr((string) $opost,4,12) . "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00";
		$this->add('post', $post);
		...
		$hmtxstr = '';	// contours in compound glyph
		$maxComponentElements = 0;		// levels of recursion, set to 0 if font has only simple glyphs
		$this->glyphdata = [];
		...
		foreach($subsetglyphs AS $originalGlyphIdx => $uni) {
			...
			if ($glyfLength < $this->maxStrLenRead) {
				$data = substr((string) $glyphData,$glyphPos,$glyphLen);
			}
			else {
			...
			if ($glyphLen > 0) {
				$up = unpack("n", substr((string) $data,0,2));
			}
			...
			if ($glyphLen > 2 && ($up[1] & (1 << 15)) ) {	// If number of contours <= -1 i.e. composite glyph
				...
				while ($flags & GF_MORE) {
					...
					$up = unpack("n", substr((string) $data,$pos_in_glyph,2));
					$flags = $up[1];
					$up = unpack("n", substr((string) $data,$pos_in_glyph+2,2));
					$glyphIdx = $up[1];
	...
	function getGlyphData($originalGlyphIdx, &$maxdepth, &$depth, &$points, &$contours): void {
		$depth++;
	...
	function getGlyphs($originalGlyphIdx, &$start, array &$glyphSet, array &$subsetglyphs): void {
		$glyphPos = $this->glyphPos[$originalGlyphIdx];
		$glyphLen = $this->glyphPos[$originalGlyphIdx + 1] - $glyphPos;
		if (!$glyphLen) { 
			return;
	...
	function getHMTX($numberOfHMetrics, $numGlyphs, array &$glyphToChar, $scale): void {
	...
	function getHMetric($numberOfHMetrics, $gid): string|false {
		$start = $this->seek_table("hmtx");
	...
	function getLOCA(?string $indexToLocFormat, $numGlyphs): void {
	...
	// CMAP Format 4
	function getCMAP4($unicode_cmap_offset, array &$glyphToChar, array &$charToGlyph ): void {
		$this->maxUniChar = 0;
	...
	function endTTFile(&$stm) {
		...
		foreach ($tables AS $tag=>$data) {
			...
			$stm .= pack("NN", $offset, strlen((string) $data));
			$paddedLength = (strlen((string) $data)+3)&~3;
